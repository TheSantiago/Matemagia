...
namespace AppAdvisory.MathGame
{
	public class GameManager : MonoBehaviour 
	{
        ...
        public int[,] Table = new int [9, 10];
        public int count = 0;
        public int TableFillCount = 0;

        public int TheDigimon = 0;
		...
		private void StartGame()
		{
            ...
            else if (hardness.text == "SALTADAS")
            {
                timeTotalGame = 25;
                timeMalus = 3;
                timeBonus = 5;
            ...
            else if (hardness.text == "EN ORDEN")
            {
                timeTotalGame = 25;
                timeMalus = 3;
                timeBonus = 5;
        ...
        void GetTheDigit()
        {
            TheDigimon = FindObjectOfType<MenuBarouch.MenuManager>().TheDigit;
        }
		...
		void ChooseOperator()
		{
            ...
            if (hardness.text == "FÁCIL")
            {
                operateur = UnityEngine.Random.Range(0, 1);
            ...
            {
                FatherForgiveMe(TheDigimon);
            }


        }

        void FatherForgiveMe(int beam)
        {
            if (beam != 0)
            {
                ...
                if (count < 9)
                {
                    ...
                    num2 = count + 1;
                    result = num1 * num2;
                    SetTableText(num1, num2, 2, result);
                }
                else
                {
                    GameOver();
        ...
        void RandomTable()
        {
            int num1 = 0, num2 = 0,result = 0,fil=0,col=0;
            if (count < 81)
            {
                while (true)
                {
                    fil = UnityEngine.Random.Range(0, 9);
                    col = UnityEngine.Random.Range(1, 10);
                    num1 = Table[fil, 0];
                    num2 = Table[fil, col];
        ...
        private void SetTableText(int n1, int n2, int oper, int result)
        {
            ...
            questionNumber1.transform.parent.Find("Selected").gameObject.SetActive(false);
            questionOperator.transform.parent.Find("Selected").gameObject.SetActive(false);
            questionNumber2.transform.parent.Find("Selected").gameObject.SetActive(false);
            questionResult.transform.parent.Find("Selected").gameObject.SetActive(true);
		...
		private void SetText(int n1, int n2, int oper, int result)
		{
			...
			{
				reponses [0].text = "+";
				reponses [0].fontSize = 170;
				reponses [1].text = "-";
				reponses [1].fontSize = 170;
				reponses [2].text = "x";
                reponses[2].fontSize = 150;
                reponses [3].text = "÷";
                reponses[3].fontSize = 150;
            } 
		}
